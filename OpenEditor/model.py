...
class Manipulater(nn.Module):
	def __init__(self, size, channels, n_latent, lr_mlp=0.01):
		super().__init__()	

		e_dim = cfg.TEXT.EMBEDDING_DIM
		w_dim = cfg.GAN.W_DIM

		self.filter = nn.Sequential(
			EqualLinear(w_dim, w_dim, lr_mul=lr_mlp, activation='fused_lrelu'), 
			EqualLinear(w_dim, w_dim, lr_mul=lr_mlp, activation='fused_lrelu'), 
			nn.Sigmoid()
		)

		self.pred_alpha = nn.Sequential(
			EqualLinear(w_dim, w_dim, lr_mul=lr_mlp, activation='fused_lrelu'),
			EqualLinear(w_dim, n_latent, lr_mul=lr_mlp, activation='fused_lrelu'),
			nn.Sigmoid()
		)
	...
	def forward(
		...
		dlat_src = netG.mapping(netG.pixel_norm(netG.fc1(sent_src)))
		dlat_tar = netG.mapping(netG.pixel_norm(netG.fc1(sent_tar)))

		gate = self.filter(dlat_src * dlat_tar)
		dlat_tar2 = dlat_tar * gate  # (N,wdim)

		alpha = self.pred_alpha(dlat_tar2).unsqueeze(-1)  # N,L,1

		# Apply truncation trick.
		if truncation < 1:
			dlat_src = truncation_latent + truncation * (dlat_src - truncation_latent)
			dlat_tar2 = truncation_latent + truncation * (dlat_tar2 - truncation_latent)
		
		# Broadcast
		if dlat_src.ndim < 3:
			dlat_src = dlat_src.unsqueeze(1).repeat(1, netG.n_latent, 1)
			dlat_tar2 = dlat_tar2.unsqueeze(1).repeat(1, netG.n_latent, 1)

		dlatents = (1 - alpha) * dlat_src + alpha * dlat_tar2

		# Noise
		if noise is None:
			if randomize_noise:
				noise = [None] * netG.num_layers
			else:
				noise = [
					getattr(netG.noises, f'noise_{i}') for i in range(netG.num_layers)
				]

		# Generator
		i_feat = []

		out = netG.const_input(dlatents)

		out = netG.conv1(out, dlatents[:, 0], noise=noise[0])
		
		i_feat.append(out)

		skip = netG.to_rgb1(out, dlatents[:, 1])
		
		i = 1
		for conv1, conv2, noise1, noise2, to_rgb in zip(
			netG.convs[::2], netG.convs[1::2], noise[1::2], noise[2::2], netG.to_rgbs
		):
		...
		return image, gate, alpha, i_feat
...
class Generator(nn.Module):
	...
	def forward(
		...
		if return_latents:
			return image, dlatents, i_feat

		else:
			return image, None, i_feat
